...
# ---- Live-server gates: ONE boot, parity + trace --------------------------
# --slots 1 lets --ctx auto size past the trace gate's ~60k cancel prompt.
server_stop
if ! server_boot "$EVID/server.log" --constrain-tools --slots 1 \
        --trace "$EVID/trace.jsonl"; then
    fail "server boot (see $EVID/server.log)"
