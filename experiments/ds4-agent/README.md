...
complete file or replacement, and EOS is the only completion boundary—no JSON
escaping, XML-like closing marker, or byte count is generated by the model.
The text CLI mirrors these deltas immediately, with a status banner, so a long
file generation does not look frozen; internal raw events remain hidden from
JSONL. Cancellation, infrastructure failure, context exhaustion, or reaching
the token bound without EOS discards the complete buffer and causes no side
effect, even though a text user may already have seen the discarded preview.
Empty EOS-complete payloads remain valid for empty-file creation and deletion
edits.

Before whole-file write publication, the harness removes one unambiguous
three-backtick outer Markdown fence only for a known source-code extension when
its language label matches the extension (or is empty), its closer is the final
line, and there is no surrounding prose. Edit fragments are never normalized:
a fenced block may be intentional inside a source string or comment. Markdown
paths, mismatched labels, and ambiguous output also remain byte-exact. Write
fence removal is reported to the terminal and in the retained tool result;
transcript accounting still charges the original model bytes. The final
mutating tool repeats all validation under its atomic race defenses; preflight
and fence repair are control-plane preparation, not authority.
