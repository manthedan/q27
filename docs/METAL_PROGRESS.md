...
4. **Production attention:** decode FP16 online-softmax landed (2026-07-14, `1824 → 65 µs` per dispatch at ctx 128, flat to ctx 4096, no probability scratch), and the chunk-causal FP16 prefill kernel now mirrors it (2026-07-14 evening; bit-exact vs serial decode attention, FP16 probability scratch eliminated, under 2% of chunk time out to ctx 480). The prefill-side GEMM attribution and rewrite also landed (see item 9: `5.7 → 22.9 tok/s` synthetic chunk ceiling). Remaining: turbo3 decode and turbo3 chunk equivalents, GQA KV reuse, and long-context cache-block scheduling.
...
The prefill side of item 4 is done (attribution bench, tiled simdgroup-matrix GEMM, online-softmax chunk attention; synthetic chunk ceiling `5.7 → 22.9 tok/s`). The next targets, in leverage order: (1) turbo3 chunk attention online-softmax — the last probability-scratch user on the prefill path and the gatekeeper for the 32K+ retrieval/perplexity runs in item 5, which are now plausible at ~20 tok/s prefill; (2) turbo3 decode attention and GQA KV reuse; (3) the MTP partial-acceptance commit re-encode and GPU-resident drafting (item 6 details); (4) residual chunk-GEMM headroom (still ~25 GB/s effective weight stream against 67–90 GB/s for decode GEMV — half-precision staging and double-buffered K-tiles are the untried levers).
