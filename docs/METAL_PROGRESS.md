...
| 9 | Performance | **In progress** | mmap views, command batching, fused RMSNorm+quantization, packed-dot GEMV (aggregate 35.6→84.8 GB/s synthetic; 128-token decode 1.22→3.01 tok/s, inside the 3–5 tok/s target band), a 3.29x N=12 simdgroup projection kernel, per-dispatch GPU profiling (`Q27_METAL_PROFILE`), a synthetic full-decode-step bench, online-softmax decode attention (1.82 ms→65 µs per dispatch at ctx 128), and widened F16 pair projections landed; resident-weight decode ceiling 245→218 ms/token; prefill GEMM bandwidth and tiled prefill/turbo3 attention remain |
...
4. **Production attention:** decode FP16 online-softmax landed (2026-07-14, `1824 → 65 µs` per dispatch at ctx 128, flat to ctx 4096, no probability scratch). Remaining: turbo3 decode equivalent, GQA KV reuse, tiled chunk-causal prefill attention (this is where the prefill scratch buffer actually dies), and long-context cache-block scheduling.
...
The immediate implementation target is the prefill side of item 4: attribute the ~15 GB/s effective chunk bandwidth with the new profiler (chunked GEMM is the biggest bandwidth outlier now that decode GEMV runs 67–90 GB/s), then tile chunk-causal prefill attention. Decode-side, the biggest remaining structural costs are the MTP partial-acceptance commit re-encode and per-lane synchronized drafting (item 6 details).
