...
| 9 | Performance | **In progress** | mmap views, command batching, fused RMSNorm+quantization, packed-dot GEMV (aggregate 35.6→84.8 GB/s synthetic; 128-token decode 1.22→3.01 tok/s, inside the 3–5 tok/s target band), and a 3.29x N=12 simdgroup projection kernel landed; non-GEMV per-token overhead attribution and tiled attention remain |
...
Remaining high-impact work, reordered by measured leverage: (1) attribute the residual per-token gap — at `3.01 tok/s` a token costs ~330 ms while the GEMV share is roughly 200 ms, so ~40% now goes to small kernels (48 under-occupied f16 alpha/beta matvecs dispatch only 1,024 threads each), attention, per-token sync, and dispatch overhead; (2) tiled attention (item 4); (3) remaining GEMV headroom toward ~100 GB/s; (4) removing the partial-round MTP commit re-encode and GPU-resident drafting/acceptance/sampling; (5) Instruments attribution of the ~15 GB/s effective chunk (prefill) bandwidth. GDN-stage fusion and dispatch-count reduction were previously listed as co-equal levers; at the old `1.22 tok/s` they were noise, and they only matter now as part of the item-(1) attribution.
...
3. **Greedy hot-path optimization:** ✅ largely done (2026-07-14). Packed-dot Q4/Q8 GEMV landed (synthetic aggregate `35.6 → 84.8 GB/s`; warm 128-token decode `1.22 → 3.01 tok/s`, inside the provisional `3–5 tok/s` band; 16-token CUDA gate byte-exact). Remaining: attribute the ~40% non-GEMV share of token time (under-occupied f16 matvecs, small kernels, per-token sync) and fuse or widen the worst offenders — measured attribution first, fusion second.
...
The immediate implementation target is item 4 (production attention), alongside the item-3 residual: attributing the ~130 ms/token that is not GEMV. The partial-acceptance commit re-encode still doubles MTP round cost, and drafting still runs one synchronized command buffer per lane. Prefill chunk passes still sustain only ~15 GB/s effective weight bandwidth; with decode GEMV now at 67–90 GB/s, the chunked GEMM is the next bandwidth outlier.
