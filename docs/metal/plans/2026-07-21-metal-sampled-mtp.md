...
Status: **PHASE 2 DONE + agent MTP** — overnight green; wall miss: verify ≈ **3–4 serial steps** (not ≈1 — see audit row), MTP net-negative on wall incl. greedy at 84–91% accept; host RS free; native agent free-decode MTP sample path landed.  
...
| 2026-07-21 | Native agent free-decode MTP | `--mtp N` + `q27_agent_engine_set_mtp_width`; temp>0 → `mtp_sample_round`, temp==0 → `mtp_round`; tool grammar gate: MTP only when grammar cannot engage (fenced bodies / tools off); `q27` wrapper: MTP opt-in via `Q27_AGENT_MTP` (no default — slower-or-inert on current packs). |
| 2026-07-21 | **Audit correction to "Why sample-mtp lost"** (independent re-derivation from the preserved logs) | verify ~0.51s/r is **~3–4 serial steps**, not ≈1 — the row above compared against the stale Jul-14 baseline (0.33–0.49s/step); the overnight's own plain leg implies a ~0.11–0.16s/tok marginal. Decomposition (non-round ≈11–12s one-time: prefill + first-touch paging of the unwired 16.5 GB pack, billed into every leg's window): steady marginals ≈ plain **6.3** > greedy-MTP **5.9** > sampled-MTP **5.5** t/s — **MTP is net-negative on wall including greedy** (round ≈3.8 plain steps, yields 3.31 tok/r sampled). Lever = fixed per-round overhead (per-lane non-batchable topk, 3 syncs × live lanes; head launch), not prompt/N. "Cold verify 4.7s" appears nowhere in the preserved traces (first verify 0.55s; first *draft* 0.77s) — it came from an unpreserved diagnostic run. **Leg 3 is underpowered** (N=32): T=1.0 (21/27) > T=0.7 (19/29) is within noise (χ² p≈0.3); only the T=1.5 sag (15/33) separates (p≈0.01); the T=0.0 leg is equality-accept (`mtp_round`), a different mechanism — don't quote the intermediate curve shape. Open evidence gaps: no end-to-end distribution A/B (plain vs sampled-MTP histogram; unit MC tests are math-level only), no live smoke of the agent MTP path (burst/ledger logic uncovered by the mock-engine selftests). |
...
| 2026-07-21 | **End-to-end distribution gate** (`tools/metal_sampled_mtp_dist.sh`, branch-review P2) | Three-leg TV gate on the official pack (N=1024, T=0.7/0.95/20): TV(plainA,plainB)=**0.6045** noise floor vs TV(plainA,mtp)=**0.6445** → **PASS** (bar 0.907). Smoke-strength: catches gross shifts (accept-mass, row-offset, lane-mapping bugs); the full seed×top_p×top_k matrix stays overnight work. New CLI `--dump-token-ids` (pure sink) feeds the histograms. Remaining sub-item: real-logits full-vs-topk-candidates `p_served` parity | *pending* |
