...
test-metal: build/test_metal build/test_metal_ops build/test_metal_stream build/test_snapshot_store_isolation
	./build/test_metal
	./build/test_metal --postcommit-failure
	./build/test_metal --profile-batch-overflow
	./build/test_metal_ops
	./build/test_metal_stream
	./build/test_snapshot_store_isolation
...
build/q27-metal-server: src/metal/metal_server.cpp src/metal/metal_engine.cpp src/metal/metal_engine.h src/metal/stream_format.h src/api_common.h src/stream_split.h src/tool_preamble.h src/suffixdraft.h src/sampling.h \
                        src/metal/disk_snapshot_store.h src/metal/snapshot_evict.h \
                        src/metal/metal_backend.mm src/metal/metal_backend.h src/metal/q27_kernels.metal \
...
build/test_snapshot_store_isolation: tools/test_snapshot_store_isolation.cpp \
                                     src/metal/disk_snapshot_store.h src/metal/snapshot_evict.h | build
	$(CXX) $(CXXFLAGS) -I src/metal tools/test_snapshot_store_isolation.cpp -o $@
...
build/test_chat_completions_integration: tools/test_chat_completions_integration.cpp tools/extract_check.sh src/server.cu src/api_common.h src/toolconstrain.h src/toolgram.h src/stream_split.h src/tokenizer.h | build
	./tools/extract_check.sh
	$(CXX) $(CXXFLAGS) -Isrc tools/test_chat_completions_integration.cpp -o $@
