...
Prefill emits exact, monotonic progress at 96-token GPU boundaries, including
cached-prefix accounting, before the first generated byte. Conservative
no-progress bounds terminate pathological output explicitly as `generation
stalled`, invalidate resident reuse, and discard incomplete calls or raw
payloads without side effects. File helpers stay workspace-bounded, but
